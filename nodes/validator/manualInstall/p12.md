...
Create a directory in your user's home dir to hold some Tessellation files.
```
cd ~
mkdir tessellation
```
...
We now need to create our passphrase for our .p12 wallet on our Node. Criteria to use:
...
```
You will need:
- Your external IP address of your Node. (Marked down the beginning steps above )
- You will create a name that you would like to call your p12 file.
- You will create a passphrase you came up with...Use requirements above.
- You will create an alias that you would like to use against your .p12 file.

:::note
The Key Store passphrase `CL_STOREPASS` has been depreciated as of version 0.0.2 when creating .p12 files.

The Key passphrase `CL_KEYPASS` has been changed to `CL_PASSWORD`, for the **.p12** creation process.

We will still be using our `CL_STOREPASS` and `CL_KEYPASS` when joining the TestNet.
:::
...
| --- | --- | --- |
| Node Username | nodeadmin | If you used this name for you user while you followed these instructions, that is perfect, no need to change this variable in the example below . You will use this in the path of CL_KEYSTORE variable line. |
...
- Copy this file format exactly as you see if here, replacing the variables with your own. Starting with the `#!/bin/bash` on the very first line. 
- Use your arrow keys to navigate around.

Create a `text` file that will be saved as a `sh` bash shell script.  When executed this will add some environment variables to your local environment where the script was run.  
...
When your file looks mostly similar to the above . You will use the CTRL-O to save the file (write it out).
...
:::info REMEMBER
Please be sure to follow the passphrase guidelines above 👆
:::
