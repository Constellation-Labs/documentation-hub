...
We will press key ( no need to hit <kbd>enter</kbd> ) the number option related to the verson that best suits are needs and follows the [upgrade path](/validate/automated/nodectl-upgrade-path) accordingly.
