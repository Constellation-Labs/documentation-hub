...
To be effective as of the next release of nodectl, Constellation Network is **deprecating** support for nodectl for the following operating systems:
...
Constellation Network recommmends a new node is built verses doing an in-place upgrade of your distribution.
:::

#### The steps are simple: 
- Take your node offline
  - `leave` and `stop` your current profiles.
    - ```sudo nodectl leave -p <profile_name>```
    - ```sudo nodectl stop -p <profile_name>```
      - *If you attempt to stop before doing a graceful leave, nodectl will request you leave first and offer you the option.*
  - Keep your original VPS in place (Do not destroy.).
...
- Destroy your original outdated VPS to stop incurring charges.
...
:::caution WARNING
If you plan to use an operating system distribution version that is no longer supported by Constellation Network or the distributor, do **not** upgrade to `v2.16.0`, as upgrading may lead to unexpected results.
:::
