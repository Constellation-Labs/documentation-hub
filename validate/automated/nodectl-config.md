...
| layer0_key | `<string>` \| None | The public key of the host that we will be connecting with to create the link (`node ID`). | Yes |
...
nodectl will then rewrite the `cn-config.yaml` replacing the three elements for us.  Since obtaining the `node ID` can be a time consuming process, rewriting the `cn-config.yaml` to persist the `node ID` improves the execution time for all over commands (that may require the node ID be known).
...
| passphrase | `<string>` | The passphrase that will unlock the `p12` file that is associated with this profile. **Do not include escape sequences (double backslashes), section signs, single quotes, or double quotes in a passphrase, AND please enclose the definition passphrase within quotes in the configuration yaml.**  The warning inside this message is only for those that are manually editing the `cn-config.yaml` file, which is NOT recommended.  Instead use the [configure command](/validate/automated/nodectl-commands#configure) | Yes |
